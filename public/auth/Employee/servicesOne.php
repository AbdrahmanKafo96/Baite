...
<script>
    ...
    function handleClick(id, name, description, show, serviceID, icon) {
        console.log(`ID: ${id}, SERVICE NAME: ${name}, DESCRIPTION: ${description}, STATUS: ${show}, SERVICE ID: ${serviceID} icon: ${icon}`);
            ...
            if (data.length < 1) {
                ...
                data.map((item) => {
                    ...
                    colDiv.innerHTML = '<div id="service" class="card p-2">' +
                        ...
                        '<a onclick="handleClick(\'' + item.id + '\', \'' + item.service_name + '\', \'' + item.description + '\', \'' + item.show + '\', \'' + item.service_id + '\', \'' + item.icon + '\')" class="btn btn-outline-primary"><i class="fas fa-edit"></i><span class="fw-bold"> تعديل </span></a>' +
